functions{
   ...
   * future
   */
  matrix KalmanFilter_back(vector rhos, vector lt_discrepancies, vector R, matrix Q, matrix C,// parameters
  matrix P, vector xhat, int Time, matrix y,matrix obs){
  ...
  for (i in 1:Time)
  {
    ...
    for (j in 1:cols(y)) 
    {
      if (obs[i,j]==1.0)
      {
        ...
        est = dot_product((xhat_ + lt_discrepancies)' ,  C[j,]) ;
        er[i,j] = y[i,j] - est; 
...
data{
  ...
  */
  ...
  real prior_ind_st_var_a; // shape parameter (alpha) of inverse gamma
	...
	real prior_ind_st_cor_lkj; // LKJ shape parameter
  ...
  matrix [N,N] prior_ind_lt_cor_beta_1; // alpha shape parameter for Beta distribution  
  matrix [N,N] prior_ind_lt_cor_beta_2; // beta shape parameter for Beta distribution  
	real<lower=0> prior_sha_st_var_exp; // Scale parameter of exponential 
	real prior_sha_st_cor_lkj; // LKJ shape parameter
    ...
    bigM[(N + 1):(N + model_num_species[1] ),(2*N + 1):(3 * N)] = Ms[1:model_num_species[1],];
	  ...
	  if(M > 1){
		  for(i in 2:M){
		    ...
		    matrix[model_num_species[i], model_num_species[i]] Ms_transformation = Ms[(sum(model_num_species[1:(i-1)]) + 1):sum(model_num_species[1:i]),];
...
model{
  ...
  */
  ...
  sha_lt_raw ~ normal(0,1);
  sha_st_var ~ exponential(prior_sha_st_var_exp); // Variance 
  sha_st_cor ~ lkj_corr(prior_sha_st_cor_lkj);  // Correlation matrix
  ...
  target += priors_cor_beta(ind_lt_cor,N,prior_ind_lt_cor_beta_1,prior_ind_lt_cor_beta_2); // Correlation matrix
  for(i in 1:M){
    ind_lt_raw[i] ~ normal(0,1);
    ind_st_var[i] ~ inv_gamma(prior_ind_st_var_a, prior_ind_st_var_b);// Variance 
    ind_st_cor[i] ~ lkj_corr(prior_ind_st_cor_lkj); // Correlation matrix
  }
...
//clever thing by Durbin and Koopman
  matrix [time,(M+2) * N] latent_mean = KalmanFilter_back(AR_params,lt_discrepancies, all_eigenvalues_cov, SIGMA, bigM,SIGMA_init, x_hat, time, new_data,observation_available);
}
